    ...
    def __init__(self, startingGameState):
        ...
        for corners in self.corners:
            if self.startingPosition == corner:
                ...
                self.visitedCorners.append((corners,False))
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                #cost = self.costFn(nextState)
                for corners in cornerState:
                    if nextState == corners[0]:
                        newCornerState = newCornerState + [(corners[0], True)]
                    else:
                        newCornerState = newCornerState + [(corners[0], corners[1])]
...
def cornersHeuristic(state, problem):
    ...
    for corner in cornerState:
        if corner[1] == False:
            ...
            dist = abs(position[0] - corner[0][0]) + abs(position[1] - corner[0][1])
            if dist >= closestCornerDist:
                closestCornerDist = dist
    
    "*** YOUR CODE HERE ***"
    
    return heur + closestCornerDist # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    return len(foodGrid.asList()) + maxDistance
    "*** YOUR CODE HERE ***"
#    return 0
